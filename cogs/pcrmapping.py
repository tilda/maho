...
class PcrMapping(Cog):
    ...
    async def add_pcrmember(self, ctx: discord.Interaction, user: discord.Member):
        ...
        await user.add_roles(
            discord.Object(self.config['pcr']['member_role']),
            reason=f'Project member addition from {ctx.user!s}')
        return await ctx.response.send_message(f':white_check_mark: Added member role to {user!s}')

    @app_commands.command()
    @app_commands.guilds(config['pcr']['guild_id'])
    async def list_pcrmembers(self, ctx: discord.Interaction):
        """Lists all project members."""
        return await ctx.response.send_message(':warning: This command is currently not implemented!')
